 ...
 * Assignment 7
 * Name:    Kenneth Cox
 * E-mail:  Integralcalc1050@gmail.com
...
public class WordGuessing {
    ...
    public static String pickWordAtRandom(String[] dictionary) {
        int randInt = new Random().nextInt(dictionary.length);
    ...
    public static boolean isIncomplete(char[] knownLetters) {
        return new String(knownLetters).contains(Character.toString(BLANK)); 
    }
     ...
     */
    public static boolean updateWithGuess(char[] knownLetters, char guessedLetter, String word) {
      if(word.contains(Character.toString(guessedLetter))){ 
        int i = word.indexOf(guessedLetter);
        while(i >= 0){
          System.out.println(i);
          knownLetters[i] = guessedLetter;  
          i = word.indexOf(guessedLetter, i + 1);
        }
        return true;
        } 
      return false;
